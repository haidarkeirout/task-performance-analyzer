# Jira Performance — Automation v3.2
...
**Data collection v3.2** after sign-in. Collection advances one bounded step per Streamlit fragment tick and resumes from its checkpoint after reruns, so partial task retrieval cannot silently discard progress.
