"""Read-only ClickUp API v2 access for the isolated ClickUp workflow.

The ClickUp adapter intentionally uses the public API only. Jira has its own
gateway and collection path and is not imported here.
"""
...
class ClickUpGateway:
    ...
    def __init__(self, token: str, session=None, sleep=time.sleep, workspace_id: str = ""):
    ...
    def _task_time_in_status(self, task_id: str):
        ...
        if not task_id:
            raise ClickUpTimeStatusUnavailable("ClickUp task ID is missing.")
        try:
            payload = self.request(f"/task/{task_id}/time_in_status")
        except ClickUpCollectionError as exc:
            raise ClickUpTimeStatusUnavailable(
                "ClickUp Total time in Status is unavailable. "
                "Enable the ClickApp (Business+) or verify the account permissions."
            ) from exc
        if not isinstance(payload, dict):
            raise ClickUpTimeStatusUnavailable(
                "ClickUp returned an unexpected Total time in Status response."
            )
        return payload

    def time_in_status(self, task_ids, progress=None):
        """Return ``{task_id: payload}`` from ClickUp's native endpoint.

        ClickUp supports a bulk request for two or more task IDs, so the
        collector makes at most one request per 100 tasks and never launches a
        browser or reads a storage-state/profile file.
        """
        ids = [str(value) for value in task_ids if value]
        if not ids:
            return {}
        if len(ids) == 1:
            return {ids[0]: self._task_time_in_status(ids[0])}
        ...
        for start in range(0, len(ids), 100):
            ...
            try:
                payload = self.request(
                    ...
                    params=[("task_ids", task_id) for task_id in batch],
                )
            except ClickUpCollectionError as exc:
                raise ClickUpTimeStatusUnavailable(
                    "ClickUp Total time in Status is unavailable. "
            ...
            if progress:
                progress(
                    "ClickUp: read Total time in Status for "
                    f"{min(start + len(batch), len(ids))} of {len(ids)} tasks..."
                )
        return result
