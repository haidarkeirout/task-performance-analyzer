...
class JiraGateway:
    ...
    def request(self, method, path, *, params=None, body=None):
        ...
        for attempt in range(4):
            try:
                response = self.session.request(method, self.base + path, params=params, json=body,
            ...
            try:
                ...
                raise CollectionError("Jira returned an unreadable response. Please try again.") from None
    ...
    def _paged(self, path, key="values", params=None):
        ...
        while True:
            ...
            if page.get("startAt", start) != start:
                raise CollectionError("Jira repeated or skipped a page. Please collect the data again.")
            total = page.get("total")
            if total is not None:
                total = int(total)
                if expected_total is not None and total != expected_total:
                    raise CollectionError("Jira data changed during collection. Please click Done again.")
            ...
            if page.get("isLast") is True or (total is not None and start == total):
                if total is not None and start != total:
                    raise CollectionError("Jira ended before all records were collected. Please try again.")
                return result
            if not values:
    ...
    def filter_reference(self, project_id):
        return self.request("POST", "/rest/api/3/jql/autocompletedata",
                            body={"projectIds": [int(project_id)], "includeCollapsedFields": False})

    def suggestions(self, field, value=""):
        return self.request("GET", "/rest/api/3/jql/autocompletedata/suggestions",
                            params={"fieldName": field, "fieldValue": value}).get("results", [])

    def validate(self, jql):
        data = self.request("POST", "/rest/api/3/jql/parse", params={"validation": "strict"},
                            body={"queries": [jql]})
        queries = data.get("queries", [])
        if not queries or queries[0].get("errors"):
            raise CollectionError("Some filters are not valid for this Jira space. Check their values or review the JQL query.")

    def search_page(self, jql, token=None, all_fields=False):
        body = {"jql": jql, "maxResults": 100 if all_fields else 50,
                "fields": ["*all"] if all_fields else ["summary", "assignee", "issuetype", "status", "priority", "resolution", "created", "duedate", "project"]}
        if token:
        ...
        if page.get("isLast") is not True and not page.get("nextPageToken"):
            raise CollectionError("Jira did not provide the next page. No partial export was prepared.")
        return page
    ...
    def all_issues(self, jql, progress=None):
        ...
        while True:
            ...
            if token in tokens:
                raise CollectionError("Jira repeated a search page. Please collect again.")
            tokens.add(token)
    ...
    def complete_issue(self, item):
        ...
        for attempt in range(2):
            ...
            # Preserve Jira's chronological page order, including equal-time transitions.
            # Lexicographic change-ID sorting would put "10" before "2" and break a chain.
            ...
            history = {"issue_key": item["key"], "snapshot": {"current_status": current},
                       "history_complete": True, "history_through": through,
        ...
        raise CollectionError("A work item changed while it was being collected. Please click Done again.")
